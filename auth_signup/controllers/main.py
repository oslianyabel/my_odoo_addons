...
class AuthSignupHome(Home):
    ...
    def web_auth_signup(self, *args, **kw):
        ...
        if "error" not in qcontext and request.httprequest.method == "POST":
            try:
                ...
                # After successful signup, ensure the current session is logged out
                # and redirect the user to the login page so they can explicitly
                # log in with their new credentials.
                try:
                    # Keep DB in session to avoid losing db choice when logging out
                    request.session.logout(keep_db=True)
                except Exception:
                    # Best-effort logout; if it fails, continue to redirect to login
                    _logger.exception("Failed to logout after signup")
                ...
                if (
                    ...
                    qcontext["error_email_exists"] = True
                else:
                    _logger.warning("%s", e)
                    qcontext["error"] = (
        ...
        if "error" not in qcontext and request.httprequest.method == "POST":
            try:
                if qcontext.get("token"):
                    self.do_signup(qcontext)
                    # If signup was performed via token (password reset flow),
                    # force logout and redirect to login so the user authenticates
                    # with the new credentials.
                    try:
                        request.session.logout(keep_db=True)
                    except Exception:
                        _logger.exception("Failed to logout after reset signup")
    ...
    def _prepare_signup_values(self, qcontext):
        ...
        if len(password) < 8 or len(password) > 12:
            password_errors.append(_("La contraseña debe tener entre 8 y 12 caracteres"))
        
        # Uppercase letter validation
        if not re.search(r'[A-Z]', password):
            password_errors.append(_("La contraseña debe contener al menos una letra mayúscula"))
        
        # Number validation
        if not re.search(r'[0-9]', password):
            password_errors.append(_("La contraseña debe contener al menos un número"))
        
        # Special character validation
        if not re.search(r'[!@#$%^&*(),.?":{}|<>]', password):
            password_errors.append(_("La contraseña debe contener al menos un carácter especial (!@#$%^&*(),.?\":{}|<>)"))
        
        if password_errors:
            raise UserError("\n".join(password_errors))
        
        # Server-side validation for name: prohibit digits and special characters
        name_val = values.get("name")
        if name_val:
            val = name_val.strip()
            if val:
                if not all(ch.isalpha() or ch in (" ", "-", "'") for ch in val):
                    raise UserError(
                        _(
                            "Por favor ingrese un nombre válido sin números ni caracteres especiales."
                        )
                    )
        # Server-side validation for email (login)
        login_val = values.get("login")
        if login_val:
            if not tools.single_email_re.match(login_val):
                raise UserError(_("Por favor ingrese un correo electrónico válido."))
